...
def merge_to_csv(
    ...
    # Put variants at the end of df, to reference main word in db
    inter_df = cxdict_freq_hsk_df[
        cxdict_freq_hsk_df["translation_en-US"].str.contains("variant of ")
        | cxdict_freq_hsk_df["translation_en-US"].str.contains("see ")
    ]
    target_rows_indexes = inter_df.index
    cxdict_freq_hsk_df = cxdict_freq_hsk_df.drop(target_rows_indexes)
    cxdict_freq_hsk_df = cxdict_freq_hsk_df.append(inter_df)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(SCRIPT_PATH, "originals/chinese_charfreq_simpl_trad.csv"),
    )
    parser.add_argument(
        "hsk3_0_wordlist_file",
        nargs="?",
    ...
    parser.add_argument(
        ...
        nargs="?",
        help="Path to hsk wordlist words directory.",
        default=os.path.join(SCRIPT_PATH, "originals/hsk2_0_wordlist.csv"),
    )
    parser.add_argument(
        "csv_radical_meaning",
        nargs="?",
        help="Path to radical with meaning file.",
        default=os.path.join(SCRIPT_PATH, "originals/radicalWithMeaning.csv"),
    )
