...
def parse_line(line):
    ...
    if len(line) <= 1:
        return 0
    translation = "__".join(line[1:-1]).replace('"', "'")
    ...
    reworked_line = "\t".join([traditional, simplified, pinyin, '"{}"'.format(translation)])

    return reworked_line


def cxdict_to_csv(cxdict_filepath, csv_cedict_headers, merge_filepath):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Converts a CXDICT u8 file into csv file.")
    parser.add_argument(
        "cxdict_filepath",
        nargs="?",
    ...
    cxdict_to_csv(cxdict_filepath, csv_cedict_headers, merge_filepath)
